		...
		public static class NerfsConfig {
			...
			public Holder<MobEffect> getEffect(RegistryAccess registryAccess) {
				if (cachedEffect == null) {
					...
					cachedEffect = effectRegistry.get(ResourceLocation.parse(nerfEffect.get())).<Holder<MobEffect>>map(h -> h)
		...
		public static class EntityBackpackAdditionsConfig {
			...
			public EntityBackpackAdditionsConfig(ModConfigSpec.Builder builder) {
				...
				entityLootTableList = builder.comment(
						...
						.defineList("entityLootTableList", this::getDefaultEntityLootTableList, () -> "mod_id:mob_name|mod_id:loot_folder/loot_table_name",
				...
				discBlockList = builder.comment("List of music discs that are not supposed to be played by entities").defineList("discBlockList",
						this::getDefaultDiscBlockList, () -> "mod_id:disc_name", mapping -> ((String) mapping).matches(REGISTRY_NAME_MATCHER));
			...
			private void initEntityLootTables() {
				...
				for (String mapping : entityLootTableList.get()) {
					...
					BuiltInRegistries.ENTITY_TYPE.getOptional(ResourceLocation.parse(entityRegistryName)).ifPresent(
							entityType -> entityLootTables.put(entityType, lootTableName.equals("null") ? null : ResourceLocation.parse(lootTableName)));
				}
			}
			...
			private List<String> getDefaultEntityLootTableList() {
				return getDefaultEntityLootMapping().entrySet().stream()
						.map(e -> BuiltInRegistries.ENTITY_TYPE.getKey(e.getKey()) + "|" + e.getValue().location()).collect(Collectors.toList());
			}
		...
		public static class NoConnectionBlocks {
			...
			NoConnectionBlocks(ModConfigSpec.Builder builder) {
				noConnectionBlocksList = builder
						...
						.defineList("noConnectionBlocks", new ArrayList<>(), () -> "mod_id:block_name",
		...
		public static class MaxUgradesPerStorageConfig implements IUpgradeCountLimitConfig {
			...
			protected MaxUgradesPerStorageConfig(ModConfigSpec.Builder builder, Map<String, Integer> defaultUpgradesPerStorage) {
				maxUpgradesPerStorageList = builder
						...
						.defineList("maxUpgradesPerStorage", convertToList(defaultUpgradesPerStorage), () -> "upgrade_name|1",
