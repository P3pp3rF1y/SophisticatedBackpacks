...
public class BackpackShapes {
	...
	private static final class JsonModelShapeProvider implements IShapeProvider {
		...
		private EnumMap<BackpackShapeHelper.Part, VoxelShape> loadPartShapes() {
			...
			Map<Identifier, JsonObject> modelCache = new HashMap<>();
		...
		private VoxelShape loadShapeFromModel(Identifier modelLocation, Map<Identifier, JsonObject> modelCache) {
			...
			Identifier parent = getParentLocation(modelJson, modelLocation);
			if (parent == null) {
				return Shapes.empty();
			}
		...
		private JsonObject resolveModelJson(Identifier modelLocation, Map<Identifier, JsonObject> modelCache) {
			return modelCache.computeIfAbsent(modelLocation, modelJsonSource::loadModelJson);
		}

		@Nullable
		private static Identifier getParentLocation(JsonObject modelJson, Identifier modelLocation) {
			if (!modelJson.has("parent")) {
				return null;
			}
			String parentName = GsonHelper.getAsString(modelJson, "parent");
			if (parentName.contains(":")) {
				return Identifier.parse(parentName);
			}
			return Identifier.fromNamespaceAndPath(modelLocation.getNamespace(), parentName);
		}
	...
	private interface ModelJsonSource {
		JsonObject loadModelJson(Identifier modelLocation);
	}
	...
	private static final class CompositeModelJsonSource implements ModelJsonSource {
		...
		@Override
		public JsonObject loadModelJson(Identifier modelLocation) {
	...
	private static final class ResourceManagerModelJsonSource implements ModelJsonSource {
		...
		@Override
		public JsonObject loadModelJson(Identifier modelLocation) {
			Identifier resourceLocation = Identifier.fromNamespaceAndPath(modelLocation.getNamespace(), "models/" + modelLocation.getPath() + ".json");
	...
	private enum PartModel {
		...
		PartModel(BackpackShapeHelper.Part part, String modelPath) {
			this.part = part;
			this.modelLocation = SophisticatedBackpacks.getIdentifier(modelPath);
		}
	}
