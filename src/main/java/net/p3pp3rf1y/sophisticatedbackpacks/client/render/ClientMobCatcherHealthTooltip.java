...
public class ClientMobCatcherHealthTooltip implements ClientTooltipComponent {
	...
	private static final int EMPTY_HEART_U = 16;
	private static final int FULL_HEART_U = 52;
	...
	private void renderInline(GuiGraphics guiGraphics, int x, int y) {
		for (int heart = 0; heart < getInlineHeartCount(); heart++) {
			renderHeart(guiGraphics, x + heart * HEART_SIZE, y, EMPTY_HEART_U);
			int remainingHealth = currentHealth - heart * 2;
			if (remainingHealth >= 2) {
				renderHeart(guiGraphics, x + heart * HEART_SIZE, y, FULL_HEART_U);
			} else if (remainingHealth == 1) {
				renderHeart(guiGraphics, x + heart * HEART_SIZE, y, HALF_HEART_U);
			}
		}
	}
	...
	private void renderCompactHearts(Font font, GuiGraphics guiGraphics, int x, int y) {
		...
		if (fullHearts > 0) {
			...
			renderHeart(guiGraphics, nextX, y, FULL_HEART_U);
			nextX += HEART_SIZE + 3;
		}
		if (hasHalfHeart) {
			renderHeart(guiGraphics, nextX, y, EMPTY_HEART_U);
			renderHeart(guiGraphics, nextX, y, HALF_HEART_U);
			nextX += HEART_SIZE + 3;
		}
		if (emptyHearts > 0) {
	...
	private void renderHeart(GuiGraphics guiGraphics, int x, int y, int u) {
		guiGraphics.blit(GUI_ICONS, x, y, u, HEART_V, HEART_SIZE, HEART_SIZE);
	}
}
