...
public class BackpackTemplateArgumentType implements ArgumentType<ResourceLocation> {
	...
	public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
		if (context.getSource() instanceof CommandSourceStack) {
			return SharedSuggestionProvider.suggest(BackpackTemplates.getTemplateNames(includeDatapackTemplates).stream().map(ResourceLocation::toString)
					.sorted(Comparator.naturalOrder()).toList(), builder);
