...
public class AccessoriesCompat implements ICompat {
	...
	public void setup() {
		...
		ModItems.ITEMS.getEntries().stream().map(RegistryObject::get).filter(BackpackItem.class::isInstance)
				.forEach(item -> AccessoriesAPI.registerAccessory(item, NOT_EQUIPPABLE_FROM_USE_ACCESSORY));
	...
	public static <T> T getFromAccessoriesStorage(LivingEntity livingEntity, String identifier, Function<AccessoriesContainer, T> getFromStorage,
			T defaultValue) {
		return AccessoriesCapability.getOptionally(livingEntity).map(cap -> {
			AccessoriesContainer storage = cap.getContainers().get(identifier);
			if (storage == null) {
				return defaultValue;
			}
