...
public class BackpackTierUpgradeRecipesMaker {
	...
	private static <T extends CraftingRecipe, U extends PropertyBasedSubtypeInterpreter> BackpackTierUpgradeDisplayRecipe createDisplayRecipe(
		...
		ResourceLocation id = recipeHolder.id().location().withPath(path -> "backpack_tier_upgrade_grouped/" + path);
		...
		RecipeHolder<CraftingRecipe> displayRecipeHolder = new RecipeHolder<>(recipeHolder.id(), new ShapedRecipe("", CraftingBookCategory.MISC,
				new ShapedRecipePattern(width, height, recipeIngredients, Optional.empty()), ClientRecipeHelper.getResultItem(recipe)));
	...
	private static NonNullList<Ingredient> copyIngredients(Collection<Optional<Ingredient>> ingredients) {
		...
		ingredients.forEach(ingredient -> ingredientsCopy.add(ingredient.orElseGet(RecipeViewerIngredients::empty)));
