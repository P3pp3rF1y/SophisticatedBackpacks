	...
	private static final ButtonDefinition.Toggle<Boolean> ANOTHER_PLAYER_CAN_OPEN = createToggleButtonDefinition(
			Map.of(
					true, getButtonStateData(new UV(176, 32), Dimension.SQUARE_16, new Position(1, 1),
							List.of(
									Component.translatable(BackpackTranslationHelper.INSTANCE.translSettingsButton("another_player_can_open.on")),
									Component.translatable(BackpackTranslationHelper.INSTANCE.translSettingsButton("another_player_can_open.on.tooltip")).withStyle(ChatFormatting.GRAY))
					),
					false, getButtonStateData(new UV(192, 32), Dimension.SQUARE_16, new Position(1, 1),
							List.of(
									Component.translatable(BackpackTranslationHelper.INSTANCE.translSettingsButton("another_player_can_open.off")),
									Component.translatable(BackpackTranslationHelper.INSTANCE.translSettingsButton("another_player_can_open.off.tooltip")).withStyle(ChatFormatting.GRAY))
					)
			));

	public BackpackMainSettingsTab(BackpackMainSettingsContainer container, Position position, SettingsScreen screen) {
		super(container, position, screen, BACKPACK_CONTEXT_TOOLTIP, Component.translatable(BackpackTranslationHelper.INSTANCE.translSettingsButton("context_backpack")),
				BackpackTranslationHelper.INSTANCE.translSettings("backpack"), BackpackTranslationHelper.INSTANCE.translSettingsTooltip("backpack"), onTabIconClicked -> new ImageButton(new Position(position.x() + 1, position.y() + 4), Dimension.SQUARE_16, ICON, onTabIconClicked));
