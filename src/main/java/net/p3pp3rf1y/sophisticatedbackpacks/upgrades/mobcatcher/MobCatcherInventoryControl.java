...
public class MobCatcherInventoryControl extends UpgradeInventoryControlBase {
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		...
		for (CapturedMob capturedMob : MobCatcherStorage.getCapturedMobs(getBackpackWrapper())) {
			...
			getRenderEntity(capturedMob).ifPresent(entity -> renderCapturedMobEntity(guiGraphics, entity, capturedMob, x, y, width, height));
	...
	public void renderErrorOverlay(GuiGraphics guiGraphics, Set<Integer> errorInventorySlots) {
		...
		for (CapturedMob capturedMob : MobCatcherStorage.getCapturedMobs(backpackWrapper)) {
			...
			Slot slot = menu.realInventorySlots.get(capturedMob.slot());
			screen.renderOverlay(guiGraphics, StorageScreenBase.ERROR_SLOT_COLOR, slot.x - 1, slot.y - 1, capturedMob.width() * 18, capturedMob.height() * 18);
		}
	}
	...
	public void renderTooltip(StorageScreenBase<?> screen, GuiGraphics guiGraphics, int mouseX, int mouseY) {
		...
		GuiHelper.renderTooltip(screen, guiGraphics, ItemStack.EMPTY, tooltipLines, Optional.of(new MobCatcherHealthTooltip(capturedMob.currentHealth(), capturedMob.maxHealth())), mouseX, mouseY);
	}
	...
	private void renderCapturedMobEntity(GuiGraphics guiGraphics, LivingEntity entity, CapturedMob capturedMob, int x, int y, int width, int height) {
		...
		int screenX = x + screen.getGuiLeft();
		int screenY = y + screen.getGuiTop();
		guiGraphics.submitEntityRenderState(renderState, getRenderScale(entity, width, height), translation, new Quaternionf(ENTITY_RENDER_ANGLE), new Quaternionf(ENTITY_CAMERA_ANGLE), screenX, screenY, screenX + width, screenY + height);
	}
	...
	private float getRenderTime(CapturedMob capturedMob) {
		return (screen.getMinecraft().player == null ? 0 : screen.getMinecraft().player.tickCount + screen.getMinecraft().getDeltaTracker().getGameTimeDeltaPartialTick(false)) + getUuidFloat(capturedMob.id(), 3) * 200F;
	}
