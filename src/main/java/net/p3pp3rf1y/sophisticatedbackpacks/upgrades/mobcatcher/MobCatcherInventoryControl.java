...
public class MobCatcherInventoryControl extends UpgradeInventoryControlBase {
	...
	private void extractCapturedMobEntity(GuiGraphicsExtractor guiGraphics, LivingEntity entity, CapturedMob capturedMob, int x, int y, int width, int height) {
		...
		guiGraphics.entity(renderState, getRenderScale(entity, width, height), translation, new Quaternionf(ENTITY_RENDER_ANGLE), new Quaternionf(ENTITY_CAMERA_ANGLE), screenX, screenY, screenX + width, screenY + height);
	}
	...
	private float getRenderTime(CapturedMob capturedMob) {
		return (screen.getMinecraft().player == null ? 0 : screen.getMinecraft().player.tickCount) + getUuidFloat(capturedMob.id(), 3) * 200F;
	}
