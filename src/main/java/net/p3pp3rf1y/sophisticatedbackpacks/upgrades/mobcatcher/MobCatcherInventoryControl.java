...
public class MobCatcherInventoryControl extends UpgradeInventoryControlBase {
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		...
		for (CapturedMob capturedMob : MobCatcherStorage.getCapturedMobs(getBackpackWrapper())) {
			...
			getRenderEntity(capturedMob).ifPresent(entity -> {
				...
				float scale = getRenderScale(entity, width, height) / entityScale;
				...
				InventoryScreen.renderEntityInInventory(guiGraphics, screenX, screenY, screenX + width, screenY + height, scale,
						translation, ENTITY_RENDER_ANGLE, ENTITY_CAMERA_ANGLE, entity);
			});
	...
	public void renderTooltip(StorageScreenBase<?> screen, GuiGraphics guiGraphics, int mouseX, int mouseY) {
		...
		guiGraphics.setTooltipForNextFrame(this.screen.getMinecraft().font, tooltipLines, Optional.of(new MobCatcherHealthTooltip(capturedMob.currentHealth(), capturedMob.maxHealth())), mouseX, mouseY);
	}
	...
	private Optional<LivingEntity> getRenderEntity(CapturedMob capturedMob) {
		...
		MobCatcherStorage.loadEntityData(livingEntity, capturedMob);
