...
import net.p3pp3rf1y.sophisticatedbackpacks.network.MobCatcherReleaseMessage;
...
public class MobCatcherInventoryControl extends UpgradeInventoryControlBase {
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		...
		for (CapturedMob capturedMob : MobCatcherStorage.getCapturedMobs(getBackpackWrapper())) {
			...
			getRenderEntity(capturedMob).ifPresent(entity -> {
				...
				InventoryScreen.renderEntityInInventory(guiGraphics, x + width / 2, (int) getRenderBottomY(entity, y, height, scale), scale,
						ENTITY_RENDER_ANGLE, ENTITY_CAMERA_ANGLE, entity);
			});
	...
	public boolean mouseClicked(double mouseX, double mouseY, int button) {
		...
		SBPPacketHandler.INSTANCE.sendToServer(new MobCatcherReleaseMessage(clickedMob.get().id()));
		return true;
	}
	...
	private float getRenderTime(CapturedMob capturedMob) {
		return (screen.getMinecraft().player == null ? 0 : screen.getMinecraft().player.tickCount + screen.getMinecraft().getFrameTime()) + getUuidFloat(capturedMob.id(), 3) * 200F;
	}
