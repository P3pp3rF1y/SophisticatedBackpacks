...
public class MobCatcherUpgradeConfig {
	...
	public MobCatcherUpgradeConfig(ModConfigSpec.Builder builder) {
		...
		basicMaxSlotCost = builder.comment("Maximum slot cost a mob may have to be captured by the basic mob catcher. This is a hard limit, not a clamp.")
				.defineInRange("basicMaxSlotCost", 18, 1, 120);
		advancedMaxSlotCost = builder.comment("Maximum slot cost a mob may have to be captured by the advanced mob catcher. This is a hard limit, not a clamp.")
				.defineInRange("advancedMaxSlotCost", 72, 1, 120);
		...
		disallowInventoryEntities = builder.comment("When true, mobs that expose inventories or container menus are not capturable.")
				.define("disallowInventoryEntities", false);
		entityBlockList = builder.comment("Entity types or entity tags prefixed with # that cannot be captured.").defineListAllowEmpty("entityBlockList",
				List::of, () -> "minecraft:wither", value -> value instanceof String s && s.matches(ENTITY_MATCHER));
		hostileOverrides = builder.comment("Entity types or entity tags prefixed with # treated as hostile by mob catcher slot cost and tier rules.")
				.defineListAllowEmpty("hostileOverrides", List::of, () -> "minecraft:enderman",
						value -> value instanceof String s && s.matches(ENTITY_MATCHER));
		passiveOverrides = builder.comment("Entity types or entity tags prefixed with # treated as passive by mob catcher slot cost and tier rules.")
				.defineListAllowEmpty("passiveOverrides", List::of, () -> "minecraft:villager",
						value -> value instanceof String s && s.matches(ENTITY_MATCHER));
		builder.pop();
	}
	...
	private static EntityMatcher getEntityMatcher(String configuredEntity) {
		if (configuredEntity.startsWith("#")) {
			return new EntityTagMatcher(TagKey.create(Registries.ENTITY_TYPE, Identifier.parse(configuredEntity.substring(1))));
		}
		return new EntityTypeMatcher(Identifier.parse(configuredEntity));
	}
	...
	private interface EntityMatcher {
		boolean matches(EntityType<?> entityType, Identifier registryName);
	}

	private record EntityTypeMatcher(Identifier matchedEntityType) implements EntityMatcher {
		@Override
		public boolean matches(EntityType<?> entityType, Identifier registryName) {
	...
	private record EntityTagMatcher(TagKey<EntityType<?>> tag) implements EntityMatcher {
		@Override
		public boolean matches(EntityType<?> entityType, Identifier registryName) {
			return entityType.builtInRegistryHolder().is(tag);
		}
	}
}
