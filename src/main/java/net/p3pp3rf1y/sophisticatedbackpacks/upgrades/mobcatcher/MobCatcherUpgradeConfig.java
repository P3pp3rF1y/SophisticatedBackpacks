...
public class MobCatcherUpgradeConfig {
	...
	public MobCatcherUpgradeConfig(ForgeConfigSpec.Builder builder) {
		...
		hostileOverrides = builder.comment("Entity types or entity tags prefixed with # treated as hostile by mob catcher slot cost and tier rules.")
				.defineListAllowEmpty("hostileOverrides", List.of("minecraft:enderman"), value -> value instanceof String s && s.matches(ENTITY_MATCHER));
		passiveOverrides = builder.comment("Entity types or entity tags prefixed with # treated as passive by mob catcher slot cost and tier rules.")
				.defineListAllowEmpty("passiveOverrides", List.of("minecraft:villager"), value -> value instanceof String s && s.matches(ENTITY_MATCHER));
		builder.pop();
	}
	...
	private static EntityMatcher getEntityMatcher(String configuredEntity) {
		if (configuredEntity.startsWith("#")) {
			return new EntityTagMatcher(TagKey.create(Registries.ENTITY_TYPE, new ResourceLocation(configuredEntity.substring(1))));
		}
		return new EntityTypeMatcher(new ResourceLocation(configuredEntity));
	}
