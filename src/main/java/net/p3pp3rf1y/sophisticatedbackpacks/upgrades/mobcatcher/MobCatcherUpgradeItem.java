...
public class MobCatcherUpgradeItem extends UpgradeItemBase<MobCatcherUpgradeWrapper> {
	...
	public UpgradeSlotChangeResult canRemoveUpgradeFrom(IStorageWrapper storageWrapper, boolean isClientSide) {
		...
		return MobCatcherStorage.getCapturedMobs(backpackWrapper).isEmpty() ? new UpgradeSlotChangeResult.Success() : new UpgradeSlotChangeResult.Fail(Component.translatable("gui.sophisticatedbackpacks.status.mob_catcher_contains_mobs"), Set.of(), Set.of(), Set.of());
	}
	...
	public UpgradeSlotChangeResult canSwapUpgradeFor(ItemStack upgradeStackToPut, int upgradeSlot, IStorageWrapper storageWrapper, boolean isClientSide) {
		...
		return MobCatcherStorage.canFitBasicTier(backpackWrapper, Config.SERVER.mobCatcherUpgrade.basicMaxSlotCost.get()) ? new UpgradeSlotChangeResult.Success() : new UpgradeSlotChangeResult.Fail(Component.translatable("gui.sophisticatedbackpacks.status.mob_catcher_mobs_need_advanced"), Set.of(), Set.of(), Set.of());
	}
}
