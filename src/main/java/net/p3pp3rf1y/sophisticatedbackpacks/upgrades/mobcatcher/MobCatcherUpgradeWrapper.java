...
public class MobCatcherUpgradeWrapper extends UpgradeWrapperBase<MobCatcherUpgradeWrapper, MobCatcherUpgradeItem>
	...
	public void addClientStorageContents(CompoundTag contents) {
		...
		if (capturedMobsTag.contains(MobCatcherStorage.CAPTURED_MOBS_TAG)) {
			Tag tag = capturedMobsTag.get(MobCatcherStorage.CAPTURED_MOBS_TAG);
			if (tag != null) {
				contents.put(MobCatcherStorage.CAPTURED_MOBS_TAG, tag.copy());
			}
		}
	}
	...
	private Optional<SoundEvent> getAmbientSound(CapturedMob capturedMob, Level level) {
		return MobCatcherStorage.getEntityType(capturedMob).map(entityType -> entityType.create(level, EntitySpawnReason.LOAD)).filter(Mob.class::isInstance)
				.map(Mob.class::cast).map(mob -> {
					MobCatcherStorage.loadEntityData(mob, capturedMob);
