...
public abstract class RefillUpgradeTab extends UpgradeSettingsTab<RefillUpgradeContainer> {
	...
	protected RefillUpgradeTab(RefillUpgradeContainer upgradeContainer, Position position, StorageScreenBase<?> screen, int slotsInRow, String upgradeName) {
		...
		filterLogicControl = addHideableChild(new FilterLogicControl<>(screen, new Position(x + 3, y + 24), getContainer().getFilterLogicContainer(), slotsInRow) {
			@Override
			protected void renderWidget(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
				super.renderWidget(poseStack, mouseX, mouseY, partialTicks);
				poseStack.pushPose();
				poseStack.translate(0, 0, 100);
				getContainer().getTargetSlots().forEach((slot, ts) -> {
					RefillUpgradeWrapper.TargetSlot targetSlot = slotBeingChanged == slot ? targetSlotBeingChanged : ts;
					drawString(poseStack, font, targetSlot.getAcronym(),
							getX() + (slot % slotsInRow) * 18 + 10, getY() + (slot / slotsInRow) * 18 + 2, DyeColor.GREEN.getTextColor());
				});
				...
				if (isMouseOver(mouseX, mouseY)) {
					...
					if (slotBeingChanged > -1) {
						...
						RefillUpgradeWrapper.TargetSlot targetSlot = getContainer().getTargetSlots().get(slot);
						if (targetSlot != null && (additionalTooltip.isEmpty() || !additionalTooltip.get(0).equals(targetSlot.getDescription()))) {
			...
			public boolean mouseScrolled(double mouseX, double mouseY, double pDelta) {
				...
				if (slotBeingChanged == -1) {
					...
					if (targetSlot != null) {
						...
						targetSlotBeingChanged = targetSlot;
					} else {
						return true;
					}
				}
