...
public class XpPumpUpgradeTab extends UpgradeSettingsTab<XpPumpUpgradeContainer> {
	...
	public static final ButtonDefinition TAKE_ALL = new ButtonDefinition(Dimension.SQUARE_18, DEFAULT_BUTTON_BACKGROUND, DEFAULT_BUTTON_HOVERED_BACKGROUND, TAKE_ALL_FOREGROUND,
			new TranslatableComponent(TranslationHelper.translUpgradeButton("take_all_experience")));
	...
	private void setStoreTooltip() {
		storeButton.setTooltip(List.of(
				new TranslatableComponent(TranslationHelper.translUpgradeButton("store_levels"), new TextComponent(String.valueOf(getContainer().getLevelsToStore())).withStyle(ChatFormatting.RED)),
				new TranslatableComponent(TranslationHelper.translUpgradeButton("store_levels.controls")).withStyle(ChatFormatting.ITALIC, ChatFormatting.DARK_GRAY))
		);
	}

	private void setTakeTooltip() {
		takeButton.setTooltip(List.of(
				new TranslatableComponent(TranslationHelper.translUpgradeButton("take_levels"), new TextComponent(String.valueOf(getContainer().getLevelsToTake())).withStyle(ChatFormatting.GREEN)),
				new TranslatableComponent(TranslationHelper.translUpgradeButton("take_levels.controls")).withStyle(ChatFormatting.ITALIC, ChatFormatting.DARK_GRAY))
		);
	}
	...
	private static class LevelSelector extends BackpackWidget {
		...
		protected void renderWidget(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
			...
			Component fullText = new TranslatableComponent(translUpgradeControl("xp_level_select"), new TextComponent(text).withStyle(ChatFormatting.WHITE)).withStyle(ChatFormatting.GRAY);
