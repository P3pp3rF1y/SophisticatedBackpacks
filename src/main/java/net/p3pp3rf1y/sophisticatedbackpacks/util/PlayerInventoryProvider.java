...
public class PlayerInventoryProvider {
	...
	private PlayerInventoryProvider() {
		addPlayerInventoryHandler(MAIN_INVENTORY, player -> PlayerInventoryHandler.SINGLE_IDENTIFIER,
				(player, identifier) -> player.getInventory().items.size(), (player, identifier, slot) -> player.getInventory().items.get(slot), true, false,
				false, false);
		addPlayerInventoryHandler(OFFHAND_INVENTORY, player -> PlayerInventoryHandler.SINGLE_IDENTIFIER,
				(player, identifier) -> player.getInventory().offhand.size(), (player, identifier, slot) -> player.getInventory().offhand.get(slot), false,
				false, false, false);
	...
	public void addPlayerInventoryHandler(String name, Function<Player, Set<String>> identifiersGetter, PlayerInventoryHandler.SlotCountGetter slotCountGetter,
		...
		playerInventoryHandlers.put(name,
				new PlayerInventoryHandler(identifiersGetter, slotCountGetter, slotStackGetter, visibleInGui, ownRenderer, accessibleByAnotherPlayer));
	...
	public boolean runOnBackpacks(Player player, BackpackInventorySlotConsumer backpackInventorySlotConsumer) {
		return runOnBackpacks(player, backpackInventorySlotConsumer, false);
	}

	public boolean runOnBackpacks(Player player, BackpackInventorySlotConsumer backpackInventorySlotConsumer, boolean onlyAccessibleByAnotherPlayer) {
		return findBackpack(player, backpackInventorySlotConsumer, onlyAccessibleByAnotherPlayer).isPresent();
	}

	public Optional<ItemStack> findBackpack(Player player, BackpackInventorySlotConsumer backpackInventorySlotConsumer) {
		return findBackpack(player, backpackInventorySlotConsumer, false);
	}

	private Optional<ItemStack> findBackpack(Player player, BackpackInventorySlotConsumer backpackInventorySlotConsumer,
