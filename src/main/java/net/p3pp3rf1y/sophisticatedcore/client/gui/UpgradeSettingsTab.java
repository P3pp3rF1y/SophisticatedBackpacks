...
public abstract class UpgradeSettingsTab<C extends UpgradeContainerBase<?, ?>> extends SettingsTabBase<StorageScreen<?>> {

	private final C upgradeContainer;

	protected UpgradeSettingsTab(C upgradeContainer, Position position, StorageScreen<?> screen, Component tabLabel, Component closedTooltip) {
	...
	protected void moveSlotsOutOfView() {
		getContainer().getSlots().forEach(slot -> slot.x = StorageScreen.DISABLED_SLOT_X_POS);
	}
