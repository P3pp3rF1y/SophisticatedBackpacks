...
class BackpackRecipeViewerDisplaySpecTest {
	...
	@Test
	void untintedIronTierRecipeUsesRealRecipeReplacementInsteadOfSyntheticFocus() {
		IRecipeViewerDisplayCatalog catalog = createCatalog();
	...
	void tierUsageChainCanReachNetheriteSmithingUpgrade() {
		...
		ItemStack ironBackpack = tintedStack(ModItems.IRON_BACKPACK.get());
	...
	void smithingSyntheticRecipeHolderUsesFocusedBaseAndResult() {
		...
		assertTrue(recipeHolder.value().baseIngredient().orElseThrow().test(tintedStack(ModItems.DIAMOND_BACKPACK.get())));
		assertFalse(recipeHolder.value().baseIngredient().orElseThrow().test(tintedStack(ModItems.IRON_BACKPACK.get())));
	...
	private static void mockClientRecipeHelper(MockedStatic<ClientRecipeHelper> clientRecipeHelper, TestRecipeResources.LoadedResources resources) {
		...
		clientRecipeHelper.when(() -> ClientRecipeHelper.transformAllRecipeHoldersOfTypeIntoMultiple(Mockito.any(), Mockito.any(), Mockito.any())).thenAnswer(invocation -> {
			...
			if (recipeClass.getName().equals(SmithingBackpackUpgradeRecipe.class.getName())) {
				RecipeHolder<SmithingBackpackUpgradeRecipe> recipeHolder = new RecipeHolder<>(ClientRecipeHelper.recipeKey(ResourceLocation.fromNamespaceAndPath(SophisticatedBackpacks.MOD_ID, "netherite_backpack")), smithingBackpackUpgradeRecipe());
				return invocation.<java.util.function.Function<RecipeHolder, List>>getArgument(2).apply(recipeHolder);
			}
			return ClientRecipeHelper.transformAllRecipeHoldersOfTypeIntoMultiple(recipeManager, recipeType, recipeClass, recipeHolder -> invocation.<java.util.function.Function<RecipeHolder, List>>getArgument(2).apply(normalizeRecipeHolder(recipeHolder)));
		});
		clientRecipeHelper.when(() -> ClientRecipeHelper.transformAllRecipeHoldersOfType(Mockito.any(), Mockito.any(), Mockito.any())).thenAnswer(invocation -> {
			RecipeType recipeType = invocation.getArgument(0);
			Class recipeClass = invocation.getArgument(1);
			return ClientRecipeHelper.transformAllRecipeHoldersOfType(recipeManager, recipeType, recipeClass, (id, recipeHolder) -> invocation.<java.util.function.BiFunction<ResourceLocation, RecipeHolder, ?>>getArgument(2).apply(id, normalizeRecipeHolder(recipeHolder)));
		});
	...
	private static RecipeHolder<?> normalizeRecipeHolder(RecipeHolder<?> recipeHolder) {
		if (recipeHolder.value() instanceof SmithingBackpackUpgradeRecipe) {
			return new RecipeHolder<>(recipeHolder.id(), smithingBackpackUpgradeRecipe());
		}
		return recipeHolder;
	}
	...
	private static BackpackUpgradeRecipe backpackUpgradeRecipe(Item sourceItem, Item resultItem) {
		List<Optional<Ingredient>> ingredients = new java.util.ArrayList<>();
		for (int i = 0; i < 9; i++) {
	...
	private static SmithingBackpackUpgradeRecipe smithingBackpackUpgradeRecipe() {
		return new SmithingBackpackUpgradeRecipe(Optional.of(Ingredient.of(Items.NETHERITE_UPGRADE_SMITHING_TEMPLATE)), Optional.of(Ingredient.of(ModItems.DIAMOND_BACKPACK.get())), Optional.of(Ingredient.of(Items.NETHERITE_INGOT)), new ItemStack(ModItems.NETHERITE_BACKPACK.get()));
	}
