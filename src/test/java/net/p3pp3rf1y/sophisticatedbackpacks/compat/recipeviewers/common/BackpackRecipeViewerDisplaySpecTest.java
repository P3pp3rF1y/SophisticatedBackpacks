...
class BackpackRecipeViewerDisplaySpecTest {
	...
	void focusedHigherTierSingleColorDyeRecipeNarrowsDyeInputAndResult() {
		SingleColorDyeRecipeSpec ironBackpackDyeSpec = createCatalog().getGroupedCraftingSpecs().stream().filter(SingleColorDyeRecipeSpec.class::isInstance)
		...
		clientRecipeHelper.when(() -> ClientRecipeHelper.transformAllRecipeHoldersOfTypeIntoMultiple(Mockito.any(), Mockito.any(), Mockito.any()))
				.thenAnswer(invocation -> {
					...
					if (recipeClass.getName().equals(SmithingBackpackUpgradeRecipe.class.getName())) {
						RecipeHolder<SmithingBackpackUpgradeRecipe> recipeHolder = new RecipeHolder<>(
								ClientRecipeHelper.recipeKey(ResourceLocation.fromNamespaceAndPath(SophisticatedBackpacks.MOD_ID, "netherite_backpack")),
					...
					return ClientRecipeHelper.transformAllRecipeHoldersOfTypeIntoMultiple(recipeManager, recipeType, recipeClass,
							recipeHolder -> invocation.<java.util.function.Function<RecipeHolder, List>>getArgument(2)
									.apply(normalizeRecipeHolder(recipeHolder)));
				});
		clientRecipeHelper.when(() -> ClientRecipeHelper.transformAllRecipeHoldersOfType(Mockito.any(), Mockito.any(), Mockito.any()))
				.thenAnswer(invocation -> {
					RecipeType recipeType = invocation.getArgument(0);
					Class recipeClass = invocation.getArgument(1);
					return ClientRecipeHelper.transformAllRecipeHoldersOfType(recipeManager, recipeType, recipeClass,
							(id, recipeHolder) -> invocation.<java.util.function.BiFunction<ResourceLocation, RecipeHolder, ?>>getArgument(2).apply(id,
									normalizeRecipeHolder(recipeHolder)));
				});
	...
	private static SmithingBackpackUpgradeRecipe smithingBackpackUpgradeRecipe() {
		return new SmithingBackpackUpgradeRecipe(Optional.of(Ingredient.of(Items.NETHERITE_UPGRADE_SMITHING_TEMPLATE)),
				Optional.of(Ingredient.of(ModItems.DIAMOND_BACKPACK.get())), Optional.of(Ingredient.of(Items.NETHERITE_INGOT)),
